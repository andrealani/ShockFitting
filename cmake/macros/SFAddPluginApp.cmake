...
##############################################################################
MACRO( SF_ADD_PLUGIN_APP APPNAME )

  # option to build it or not
  OPTION ( SF_BUILD_${APPNAME} "Build the ${APPNAME} application" ON )

  # add to list of local apps
  LIST ( APPEND SF_LOCAL_APPNAMES ${APPNAME} )
...
#   SF_DEBUG_VAR(SF_MODULES_LIST)
  ...
  IF ( SF_BUILD_${APPNAME} AND ${APPNAME}_all_mods_pres)
  ...
  IF (${APPNAME}_will_compile)
    ...
    SF_SEPARATE_SOURCES("${${APPNAME}_files}" ${APPNAME})
...
#           MESSAGE ( STATUS "${APPNAME} links to ${MPI_LIBRARIES}" )
    ...
    IF ( SF_ENABLE_INTERNAL_DEPS )
      ...
      ENDIF()
    ELSE()
      TARGET_LINK_LIBRARIES ( ${APPNAME} ${SF_KERNEL_LIBS} ${SF_KERNEL_STATIC_LIBS} ${SF_Boost_LIBRARIES} )
    ENDIF()
  ...
  LOGFILE("${APPNAME} : [${SF_BUILD_${APPNAME}}]")
