...
#include <stdio.h>
#include <iomanip>
#include "ConverterSF/CFmesh2TriangleFVM.hh"
...
ObjectProvider<CFmesh2TriangleFVM, Converter>
CFmesh2TriangleFVMProv("CFmesh2TriangleFVM");

//----------------------------------------------------------------------------//

CFmesh2TriangleFVM::CFmesh2TriangleFVM(const std::string& objectName) :
...
CFmesh2TriangleFVM::~CFmesh2TriangleFVM()
{
}

//----------------------------------------------------------------------------//

void CFmesh2TriangleFVM::setup()
{
  LogToScreen(VERBOSE, "CFmesh2TriangleFVM::setup() => start\n");

  m_prim2param.ptr()->setup();

  LogToScreen(VERBOSE, "CFmesh2TriangleFVM::setup() => end\n");
}

//----------------------------------------------------------------------------//

void CFmesh2TriangleFVM::unsetup()
{
  LogToScreen(VERBOSE, "CFmesh2TriangleFVM::unsetup()\n");
...
void CFmesh2TriangleFVM::configure(OptionMap& cmap, const std::string& prefix)
{
...
void CFmesh2TriangleFVM::convert()
{
  LogToScreen (INFO, "CFmesh2TriangleFVM::convert()\n");

  setMeshData();
  setPhysicsData();

  // read CFmesh format file
  LogToScreen(DEBUG_MIN, "CFmesh2TriangleFVM::reading CFmesh format\n");
  readCFmeshFmt();
  ...
  if (MeshData::getInstance().getVersion()=="original")
  {
   ...
   LogToScreen(DEBUG_MIN, "CFmesh2TriangleFVM::writing Triangle format\n");
   writeTriangleFmt(); 
  }
...
void CFmesh2TriangleFVM::readCFmeshFmt()
{
  // dummy variables
  ...
  for(unsigned IC=0; IC<NCLR; IC++) { 
   ...
   for(unsigned IB=0; IB<nFacB.at(IC);IB++) {
    file >> dummy >> dummy >> bndfac0 >> bndfac1 >> istate;
  ...
  else { cout << "CFmesh2TriangleFVM::error => !LIST_STATES must be 0 or 1 ";
...
void CFmesh2TriangleFVM::resizeVectors()
{
...
void CFmesh2TriangleFVM::setNodcod()
{
  int IPOIN, help;
...
void CFmesh2TriangleFVM::countnbBoundaryNodes()
{
  // write new nodcode values on the vector of the shocked mesh
  // in the fortran version this new vector is referred to index 1 (NODCOD(1))
  // here it is pushed back to the nodcod of the background mesh
...
void CFmesh2TriangleFVM::writeTriangleFmt()
{
...
void CFmesh2TriangleFVM::freeArray()
{
...
void CFmesh2TriangleFVM::setMeshData()
{
...
void CFmesh2TriangleFVM::setPhysicsData()
{
