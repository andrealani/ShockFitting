...
#ifndef ShockFitting_StandardShockFittingBeta_hh
#define ShockFitting_StandardShockFittingBeta_hh
...
/// This class defines a StandardShockFittingBeta, whose task is to read files
/// which informations about a mesh and a chemical enviroment and process
/// the data.

class StandardShockFittingBeta : public ShockFittingObj {
public:

  ///Constructor
  /// @param objectName the concrete class name
  StandardShockFittingBeta(const std::string& objectName);

  ///Destructor
  virtual ~StandardShockFittingBeta();
...
private:
  ...
  /// specifies if the heat flux will be computed by the shock-fitting
  bool m_postProHeatFlux;

  /// specifies if the median dual cell will be defined
  bool m_needMedianDualCell;
  ...
  /// command object detect discontinuities
  SConfig::SharedPtr<ShockDetector> m_shockDetector;
  ...
  // command object define median dual cell for each grid node
  SConfig::SharedPtr<MeshGenerator> m_identifyMedianDualCell;
...
#endif //ShockFitting_StandardShockFittingBeta_hh
