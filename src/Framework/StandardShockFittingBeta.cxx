...
{
  LogToScreen(VERBOSE, "StandardShockFittingBeta::configure() => start\n");
...
{
  LogToScreen(VERBOSE, "StandardShockFittingBeta::setup() => start\n");

  ShockFittingObj::setup();

  m_createTriangleFiles = m_fConverter[0].ptr();
  ...
  m_CFmeshToTriangle = m_fConverter[2].ptr();
  ...
  LogToScreen(VERBOSE, "StandardShockFittingBeta::setup() => end\n");  
}
...
{
  LogToScreen(VERBOSE, "StandardShockFittingBeta::unsetup() => start\n");

  ShockFittingObj::unsetup();
  
  LogToScreen(VERBOSE, "StandardShockFittingBeta::unsetup() => end\n");
}
...
{
  LogToScreen(VERBOSE, "StandardShockFittingBeta::process() => start\n");
  ...
  if(!m_startFiles) { m_readInputFile2->generate(); }
  ...
  LogToScreen(VERBOSE, "StandardShockFittingBeta::process() => end\n");
}
